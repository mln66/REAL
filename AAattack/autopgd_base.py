...
class APGDAttack():
        ...
        for _ in range(self.eot_iter):
            if not self.is_tf_model:
                with torch.enable_grad():
                    logits = self.model(self.normalize_f(x_adv))[0]
    ...
    def perturb(self, x, y=None, best_loss=False, x_init=None):
        ...
        if not self.is_tf_model:
            y_pred = self.model(self.normalize_f(x))[0].max(1)[1]
        else:
            y_pred = self.model.predict(self.normalize_f(x))[0].max(1)[1]
        if y is None:
...
class APGDAttack_targeted(APGDAttack):
    def __init__(
        ...
        """
        super(APGDAttack_targeted, self).__init__(predict, normalize_f, n_iter=n_iter, norm=norm,
        ...
        for target_class in range(2, self.n_target_classes + 2):
            for counter in range(self.n_restarts):
                ...
                if ind_to_fool.numel() != 0:
                    ...
                    if not self.is_tf_model:
                        output = self.model(self.normalize_f(x_to_fool))[0]
