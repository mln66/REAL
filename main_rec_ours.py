...
def train_main(args):
    ...
    if args.auxiliary == 'pi':
        train_transform = load_transform(args.dataset,
                                         mod='test')  # data augmentation is wrapped in the auxiliary loss function
    else:
    ...
    if args.pretrained:
        model = load_model(args.model, args.dataset, n_class, in_channel, save_dir=pre_dir, ).to(device)
    else:
...
def train_sub(args):
    ...
    for sub_epoch in range(args.sub_epochs):
        ...
        train_loader = torch.utils.data.DataLoader(dataset, batch_size=150, shuffle=True, num_workers=args.workers,
                                                   drop_last=False)
...
def test_main(args):
    ...
    file = open(os.path.join(save_dir, ''), 'a')
    # sys.stdout = file
    print(datetime.now())
    ...
    print('auxiliary task:{}'.format(args.auxiliary))
    # auxiliary task
    if args.auxiliary == 'rec':
        if args.shot == True:
            ...
            aux_criterion_1 = recon_criterion
            aux_criterion_2 = recon_criterion
    ...
    if args.attack is not None:
        if args.attack == 'fgsm':
            ...
            attack = partial(globals()[args.attack], epsilon=args.epsilon, step_size=args.step_size,
            ...
            attack = partial(globals()[args.attack], epsilon=args.epsilon, step_size=args.step_size,
                             num_iter=args.num_iter, purify=partial(globals()['purify'], aux_criterion=aux_criterion))
...
def parse_args():
    ...
    parser.add_argument('-n', '--name', default='ae_dr_aux', help='folder in the save path')
    parser.add_argument('-t', '--test-only', default=True, dest="test_only", help="Only test the model",
                        action="store_true")
    parser.add_argument('--sub', help="train substitute model", action="store_true")

    # model and dataset
    parser.add_argument('-m', '--model', default='ae', help='type of the model')
    parser.add_argument('-sm', '--sub-model', default='ae', help='type of the substitute model')
    parser.add_argument('-d', '--dataset', default='mnist', help='name of the dataset')

    # adversarial attacks
    parser.add_argument('-at', '--attack', default='fgsm', help="adversarial attack strategy")
    parser.add_argument('-e', '--epsilon', type=float, default=0.3, help="adversary epsilon")
    parser.add_argument('-s', '--step-size', type=float, default=0.01, help="pgd adversary step size")
    parser.add_argument('-ni', '--num-iter', type=int, default=10, help="pgd adversary number of iteration")
    ...
    # adversarial defense (purification)pfy-delta default=0.1 ,pfy-step-size  default=0.1
    parser.add_argument('-df', '--defense', default='pgd_linf', help="adversarial defense strategy")
    parser.add_argument('-aux', '--auxiliary', default='rec', help="auxiliary task for defense")
    parser.add_argument('-shot', '--shot', default=True, help="entropy loss for defense")

    parser.add_argument('-a', '--alpha', type=float, default=100, help="weight of auxiliary loss during training")
    ...
    parser.add_argument('-u', '--gpus', default='3', help='index of specified gpus')
    ...
    parser.add_argument('--lr', default=0.01, type=float, help='initial learning rate (default for sgd: 0.1)')
